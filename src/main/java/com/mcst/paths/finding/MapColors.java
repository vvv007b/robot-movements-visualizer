...
//        realityMapColors.put(new Color)
    ...
    //color1 - from legend, color2 - real color
    public boolean isColorsSimilar(Color color1, Color color2) {
        int color1Red = color1.getRed();
        int color1Green = color1.getGreen();
        int color1Blue = color1.getBlue();
        return (color2.getRed() - 1 <= color1Red && color1Red <= color2.getRed() + 1 &&
                color2.getGreen() - 1 <= color1Green && color1Green <= color2.getGreen() + 1 &&
                color2.getBlue() - 1 <= color1Blue && color1Blue <= color2.getBlue() + 1);
    }
    ...
    public int getPassability(Color color) {
        if (streetMapsColors.containsKey(color)) {
            ...
            for (Entry<Color, Integer> entry : streetMapsColors.entrySet()) {
                ...
                if (isColorsSimilar(entryColor, color)) {
