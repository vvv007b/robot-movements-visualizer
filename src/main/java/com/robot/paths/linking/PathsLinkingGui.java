...
import com.robot.paths.finding.Hypervisor;
...
public class PathsLinkingGui extends JFrame{
    ...
    public PathsLinkingGui() {
        super();
        final long startUITime = System.currentTimeMillis();
        currentCamera = null;
        createMyComponents();
...
//                }
    ...
    private void createMyComponents() {
        ...
        cameraRSlider.addChangeListener(e -> {
            currentCamera.setRadius(cameraRSlider.getValue());
            currentCamera.redrawFov();
            mapPanel.repaint();
    ...
    public void updateStatus() {
        ...
        for (Camera curCamera:TrackingSystem.getCameraList()) {
            status += "Camera " + TrackingSystem.getCameraList().indexOf(curCamera) + " see " +
                    curCamera.getTracker().getVisibleRobotsCount() + " robots<br>";
        }
        status += "</html>";
    ...
    public void inOutVectorNotification(InOutVector vector) {
        String message = (vector.getOrientation() == InOutVector.IN ? "In " : "Out ") +
                ...
                "connections: " + (vector.getOrientation() == InOutVector.IN ?
                                    vector.getPrev().size() : vector.getNext().size());
    ...
    public void setCurrentCamera(Camera camera) {
        ...
        cameraRSlider.setValue(camera.getRadius());
        cameraAzimuthTextField.setText("Camera azimuth: " + cameraAzimuthSlider.getValue());
        cameraAngleTextField.setText("Camera angle of view: " + cameraAngleSlider.getValue());
        cameraRTextField.setText("Camera radius: " + cameraRSlider.getValue());
    }
    ...
    /*public void drawLine(StraightLine line){
        double a = line.getA();
        double r = line.getRadius();

        Point2D point1 = new Point2D.Double(0, r / CurvesMath.sin(CurvesMath.toRadians(a))),
                point2 = new Point2D.Double(r / CurvesMath.cos(CurvesMath.toRadians(a)), 0),
                point3 = new Point2D.Double(mapPanel.getWidth(), (r - mapPanel.getWidth() *
                        CurvesMath.cos(CurvesMath.toRadians(a))) / CurvesMath.sin(CurvesMath.toRadians(a))),
                point4 = new Point2D.Double((r - mapPanel.getHeight() * CurvesMath.sin(CurvesMath.toRadians(a))) /
                        CurvesMath.cos(CurvesMath.toRadians(a)), mapPanel.getHeight());
